...
s=[99.0,99.0] #Initial x,y,z=0
def equation(s):
    #return (3*s[0] + 2*s[1] + 5*z) #return 3x + 2y + 5z
...
def neighbour(s):
    ...
    x = random.uniform(0.0,99.99)
    y = random.uniform(0.0,99.99)
    #z = random.uniform(0.0,99.99)
    while True:
        #if x+y > 10 or 2*x + z > 9 or y + 2*z > 11:
...
print(f'Result : {equation(result):.2f} , x : {result[0]:.2f} , y : {result[1]:.2f}')
